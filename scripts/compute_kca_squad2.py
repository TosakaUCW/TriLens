#!/usr/bin/env python3
"""KCA diagnostic on SQuAD2.0 with paired answer candidates.

right = gold_answers[0]  (or "No answer." when the question is impossible)
hallu = plausible_answers[0]  (or a random wrong answer from the pool)

Both candidates are short answer strings drawn from the same distribution,
so response-length confound is minimal — the ideal setting for evaluating
per-module logit-lens signals without length leakage.
"""
import argparse
import json
import random
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KCA diagnostic on SQuAD2.0.")
            ...
            for i, rec in enumerate(records):
                ...
                for cand_idx, cand in enumerate(candidates):
                    ...
                    row = {
                        ...
                        "is_impossible": bool(rec.get("is_impossible", False)),
                        "num_layers": captures.n_layers,
